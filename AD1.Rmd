...
Below I plot end-of-month VIX from February 1990 to December 2016. It appears that this time-series exhibits cycles, as high values are observed in the years immediately preceding and following recessions (shaded in gray), while low values are observed the further away we are from a recession.
...
I then plot the forecasts computed by the two Arima models over for the period of January 2017 to May 2023 to compare the results obtained by the forecasts with the actual values of the change in the Federal Funds Effective Rate composite indicator. It is difficult to visually assess which forecast is better since they produced similar results.
...
Finally, I use the VAR model to produce an additional forecast over a two-year horizon for both the VIX and the change in the Federal Funds Effective Rate. The results indicate that the VIX will steadily grow over the next two years, reaching ~18.84 in June 2025. On the other hand, the change in the Federal Funds Effective Rate will experience a more volatile behavior, as it is expected to experience months of growth as well as months of decline. It is predicted to grow by ~3.78% in June 2025.

```{r}
var.predict = predict(object=y_model3, n.ahead=24)
dev.new(width = 7, height = 100)
plot(var.predict)
```

```{r}
library('magick')
img<-image_read("/Users/sandinatatu/Desktop/Screenshot 2023-07-11 at 2.47.48 AM.png")
plot(img)
```
